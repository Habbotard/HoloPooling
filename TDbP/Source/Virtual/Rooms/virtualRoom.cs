...
namespace Holo.Virtual.Rooms
{
    ...
    public class virtualRoom
    {
        ...
        public virtualRoom(int roomID, bool isPublicroom)
        {
            ...
            Database dbClient = new Database(true, false, 58);

            string roomModel = dbClient.getString("SELECT model FROM rooms WHERE id = '" + roomID + "'");
            DataRow dRow = dbClient.getRow("SELECT door_x,door_y,door_h,door_z,heightmap FROM room_modeldata WHERE model = '" + roomModel + "'");
            ...
            if (isPublicroom)
            {                
                string[] Items = dbClient.getString("SELECT publicroom_items FROM room_modeldata WHERE model = '" + roomModel + "'").Split("\n".ToCharArray());
                ...
                DataTable dTable = dbClient.getTable("SELECT object,x,y,goalx,goaly,stepx,stepy,roomid,state FROM room_modeldata_triggers WHERE model = '" + roomModel + "'");
                ...
                if (dbClient.findsResult("SELECT specialcast_interval FROM room_modeldata WHERE model = '" + roomModel + "' AND specialcast_interval > 0"))
                {
                ...
                hasSwimmingPool = dbClient.findsResult("SELECT swimmingpool FROM room_modeldata WHERE model = '" + roomModel + "' AND swimmingpool = '1'");

                if (dbClient.findsResult("SELECT id FROM games_lobbies WHERE id = '" + roomID + "'"))
                {
            ...
            else
            {
                ...
                DataTable dTable = dbClient.getTable("SELECT id,tid,x,y,z,h,var,wallpos FROM furniture WHERE roomid = '" + roomID + "' ORDER BY h ASC");
                foreach (DataRow dbRow in dTable.Rows)
        ...
        internal void addUser(virtualUser User)
        {
            ...
            Database dbClient = new Database(true, false, 69);
            ...
            else
            {
                ...
                {
                    User.roomUser.gamePoints = dbClient.getInteger("SELECT " + Lobby.Type + "_totalpoints FROM users WHERE id = '" + User.userID + "'");
            ...
            {
                string groupBadge = dbClient.getString("SELECT badge FROM groups_details WHERE id = '" + User._groupID + "'");
        ...
        internal string Groups
        {
            get
            {
                ...
                sb.Append(" OR");
                Database dbClient = new Database(true, true, 70);
                DataTable dTable = dbClient.getTable("SELECT id,badge FROM group_details WHERE" + sb.ToString().Substring(3) + " 'a' = 'b'", null);
                                ...
                                #region Swimming pool triggers
                                {
                                    ...
                                    else if (roomUser.swimOutfit != "") // User wears a swim outfit and hasn't entered a swimming pool clothing booth
                                    {
                                        if (Trigger.Object == "door" && roomUser.User._Tickets != 33333) // User has entered the diving board elevator
                                        {
                                            ...
                                            Database dbClient = new Database(true, true, 71);
                                            dbClient.runQuery("UPDATE users SET tickets = tickets - 1 WHERE id = '" + roomUser.userID + "' LIMIT 1");
                                        }
        ...
        {
            Database dbClient = new Database(true, true, 57);
            DataColumn dCol = dbClient.getColumn("SELECT id FROM roombots WHERE roomid = '" + this.roomID + "'");
        ...
        internal class FloorItemManager
        {
            ...
            internal void removeItem(int itemID, int ownerID)
            {
                if(_Items.ContainsKey(itemID))
                {
                    ...
                    Database dbClient = new Database(true, true, 59);
            ...
            internal void placeItem(int itemID, int templateID, int X, int Y, byte typeID, byte Z)
            {
                ...
                try
                {
                    ...
                    Database dbClient = new Database(true, false, 60);
            ...
            internal void relocateItem(int itemID, int X, int Y, byte Z)
            {
                try
                {
                    ...
                    Database dbClient = new Database(true, true, 61);
            ...
            internal void toggleItemStatus(int itemID, string toStatus, bool hasRights)
            {
                ...
                Database dbClient = new Database(false, true, 62);
                dbClient.addParameterWithValue("var", toStatus);
                dbClient.Open();
        ...
        internal class WallItemManager
        {
            ...
            internal void addItem(int itemID, int templateID, string wallPosition, string Var, bool Place)
            {
                ...
                if (_Items.Contains(itemID) == false)
                {
                    ...
                    if (Place)
                    {
                        ...
                        Database dbClient = new Database(true, false, 63);
            ...
            internal void removeItem(int itemID, int ownerID)
            {
                if (_Items.ContainsKey(itemID))
                {
                    ...
                    _Items.Remove(itemID);
                    Database dbClient = new Database(true, true, 64);
                    if (ownerID > 0)
            ...
            internal void toggleItemStatus(int itemID, int toStatus)
            {
                ...
                Database dbClient = new Database(true, true, 65);
                dbClient.runQuery("UPDATE furniture SET var = '" + toStatus + "' WHERE id = '" + itemID + "' LIMIT 1");
            }
            /// <summary>
            /// Returns a string with all the virtual wallitems in this item manager.
            ...
            internal string moodLight_GetSettings(int roomID)
            {
                    try
                    {
                        Database dbClient = new Database(true, true, 66);
                        string[] itemSettings = (string[])dbClient.getRow("SELECT preset_cur,preset_1,preset_2,preset_3 FROM furniture_moodlight WHERE roomid ='" + roomID + "'").ItemArray;
            ...
            {
                Database dbClient = new Database(true, false, 67);
                int itemID = dbClient.getInteger("SELECT id FROM furniture_moodlight WHERE roomid ='" + roomID + "'");
                string newPresetValue;
                if (isEnabled == false)
                {
                    string curPresetValue = dbClient.getString("SELECT var FROM furniture WHERE id ='" + itemID + "'");
                ...
                DataRow dRow = dbClient.getRow("SELECT wallpos,var FROM furniture WHERE id = '" + itemID + "'");
        ...
        private void handleSpecialCasts(object o)
        {
            try
            {
                Database dbClient = new Database(true, true, 68);
                DataRow dRow = dbClient.getRow("SELECT specialcast_emitter,specialcast)interval,specialcast_rnd_min, specialcast_rnd_max FROM room_modeldata WHERE model = '" + (string)o + "'");
                string Emitter = Convert.ToString(dRow["specialcast_emitter"]);
