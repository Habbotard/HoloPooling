...
namespace Holo.Virtual.Rooms.Bots
{
    ...
    internal class virtualBot
    {
        ...
        internal virtualBot(int botID, int roomUID, virtualRoom Room)
        {
            ...
            Database dbClient = new Database(true, false, 52);
            DataRow dRow = dbClient.getRow("SELECT name,mission,figure,x,y,z,freeroam,message_noshouting FROM roombots WHERE id = '" + botID + "'");
            ...
            DataColumn dCol = dbClient.getColumn("SELECT text FROM roombots_texts WHERE id = '" + botID + "' AND  type = 'say'");
