...
namespace Holo.Virtual.Rooms.Games
{
    internal class Game
    {
        ...
        internal void startGame()
        {
            ...
            Database dbClient = new Database(true, false, 53);
            ...
            for (int i = 0; i < Teams.Length; i++)
            {
                ...
                DataRow dRow = dbClient.getRow("SELECT x,y,z FROM games_maps_playerspawns WHERE type = '" + Lobby.Type + "' AND mapid = '" + mapID + "' AND teamid = '" + i + "'");
        ...
        private void countDownTicker()
        {
            ...
            Database dbClient = new Database(true, true, 54);
            for (int i = 0; i < Teams.Length; i++)
                foreach (gamePlayer Player in Teams[i])
                {
        ...
        private void updateLoop()
        {
            if (isBattleBall)
            {
                ...
                Database dbClient = new Database(true, true, 55);
                for (int i = 0; i < teamAmount; i++)
                {
