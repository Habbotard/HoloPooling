    ...
    public static class recyclerManager
    {
        ...
        public static void Init(bool Update)
        {
            if (Config.getTableEntry("recycler_enable") == "1")
            {
                ...
                Database dbClient = new Database(true, true, 23);
                DataTable dTable = dbClient.getTable("SELECT rclr_cost, rclr_reward FROM system_recycler");
        ...
        public static void createSession(int userID, int itemCount)
        {
            ...
            Database dbClient = new Database(false, true, 24);
            dbClient.addParameterWithValue("userID", userID);
            dbClient.addParameterWithValue("dateTime", DateTime.Now.ToString());
            dbClient.addParameterWithValue("rewardTemplateID", rewardTemplateID);
            dbClient.Open();
        ...
        {
            Database dbClient = new Database(true, false, 25);
        ...
        public static void rewardSession(int userID)
        {
            ...
            Database dbClient = new Database(false, true, 18);
            dbClient.addParameterWithValue("reward", sessionRewardID(userID));
            dbClient.addParameterWithValue("userID", userID);
            dbClient.Open();
            dbClient.runQuery("INSERT INTO furniture(tid,ownerid) VALUES ('@reward','@userID')");
        ...
        public static int passedMinutes(int userID)
        {
            try
            {
                Database dbClient = new Database(true, true, 19);
                TimeSpan Span = DateTime.Now - DateTime.Parse(dbClient.getString("SELECT session_started FROM users_recycler WHERE userid = '" + userID + "' LIMIT 1"));
        ...
        public static string sessionString(int userID)
        {
            if(Config.enableRecycler == false)
                return "H";
            Database dbClient = new Database(true, true, 20);
        ...
        {
            Database dbClient = new Database(true, true, 21);
        ...
        {
            Database dbClient = new Database(true, true, 22);
            return dbClient.getInteger("SELECT session_reward FROM users_recycler WHERE userid = '" + userID + "'");
        }
        #endregion
    }
}
