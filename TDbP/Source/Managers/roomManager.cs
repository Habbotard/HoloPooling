...
namespace Holo.Managers
{
    ...
    public static class roomManager
    {
        ...
        {
            Database dbClient = new Database(true, true, 26);
        ...
        public static class moodlight
        {
            ...
            public static string getSettings(int roomID)
            {
                try
                {
                    Database dbClient = new Database(true, true, 27);
                    DataRow dRow = dbClient.getRow("SELECT preset_cur,preset_1,preset_2,preset_3 FROM furniture_moodlight WHERE roomid = '" + roomID + "'");
            ...
            {
                Database dbClient = new Database(true, false, 28);
                ...
                else
                {
                    ...
                    dbClient.Close();
                    dbClient = new Database(false, false, 29);
                    dbClient.addParameterWithValue("var", newPresetValue);
                    dbClient.addParameterWithValue("preset", bgState.ToString() + ", " + presetColour + ", " + alphaDarkF.ToString());
                    dbClient.Open();
                ...
                string wallPosition = dbClient.getString("SELECT wallpos FROM furniture WHERE id = '" + itemID + "'");
                dbClient.Close();
