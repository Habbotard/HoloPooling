...
namespace Holo.Managers
{
    ...
    public static class eventManager
    {
        ...
        {
            Database dbClient = new Database(true, false, 15);
            while (true)
            {
                for (int i = 0; i < categoryAmount; i++)
                {
        ...
        public static void createEvent(int categoryID, int userID, int roomID, string Name, string Description)
        {
            if (Events[categoryID - 1].Contains(roomID) == false)
            {
                ...
                Database dbClient = new Database(false, true, 12);
                dbClient.addParameterWithValue("ename", Name);
                dbClient.addParameterWithValue("edescription", Description);
                dbClient.Open();
        ...
        public static void removeEvent(int roomID)
        {
            for (int i = 0; i < categoryAmount; i++)
            {
                ...
                {
                    Database dbClient = new Database(true, true, 13);
        ...
        public static void editEvent(int categoryID, int roomID, string Name, string Description)
        {
            if (Events[categoryID - 1].Contains(roomID))
            {
                ...
                Database dbClient = new Database(true, true, 14);
                dbClient.runQuery("UPDATE events SET name = '" + Name + "',description = '" + Description + "',category = '" + categoryID + "',date = '" + DateTime.Now.TimeOfDay.ToString() + "' WHERE roomid = '" + roomID + "'");
