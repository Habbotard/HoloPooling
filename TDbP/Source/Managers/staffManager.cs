...
namespace Holo.Managers
{
    ...
    public static class staffManager
    {
        ...
        {
            Database dbClient = new Database(false, true, 36);
            dbClient.addParameterWithValue("action", Action);
            dbClient.addParameterWithValue("userid", userID);
            dbClient.addParameterWithValue("targetid", targetID);
            dbClient.addParameterWithValue("message", Message);
            dbClient.addParameterWithValue("time", DateTime.Now.ToString());
            dbClient.Open();
