...
namespace Holo.Managers
{
    ...
    public static class stringManager
    {
        ...
        public static void Init(string langExtension, bool Update)
        {
            ...
            Database dbClient = new Database(true, true, 37);
            if (dbClient.Ready)
            {
                DataTable dTable = dbClient.getTable("SELECT stringid, var_" + langExt + " FROM system_strings ORDER BY id ASC");
                string[] langKeys = new string[dTable.Rows.Count];
                string[] langVars = new string[dTable.Rows.Count];
        ...
        public static void initFilter(bool Update)
        {
            if (Config.getTableEntry("wordfilter_enable") == "1")
            {
                ...
                Database dbClient = new Database(true, true, 38);
                DataColumn dCol = dbClient.getColumn("SELECT word FROM wordfilter");
