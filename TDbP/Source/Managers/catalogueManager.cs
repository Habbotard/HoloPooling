...
namespace Holo.Managers
{
    ...
    public static class catalogueManager
    {
        ...
        public static void Init(bool Update)
        {
            ...
            Database dbClient = new Database(true, true, 7);
            DataColumn dCol = dbClient.getColumn("SELECT indexid FROM catalogue_pages ORDER BY indexid");
        ...
        {
            Database dbClient = new Database(true, false, 11);
            DataRow dRow = dbClient.getRow("SELECT indexname,minrank,displayname,style_layout,img_header,img_side,label_description,label_misc,label_moredetails FROM catalogue_pages WHERE indexid = '" + pageID + "' LIMIT 1");
            ...
            DataTable dTable = dbClient.getTable("SELECT tid, typeid, length, width, catalogue_cost, door, tradeable, recycleable, catalogue_name, catalogue_description, name_cct, colour, top FROM catalogue_items WHERE catalogue_id_page = '" + pageID + "' ORDER BY catalogue_id_index ASC");
        ...
        public static string getPageIndex(byte userRank)
        {
            try
            {
                ...
                Database dbClient = new Database(true, true, 6);
                DataColumn dCol = dbClient.getColumn("SELECT indexname FROM catalogue_pages WHERE minrank <= '" + userRank + "' ORDER BY indexid ASC");
        ...
        public static void handlePurchase(int templateID, int receiverID, int roomID, int decorID, int presentBoxID)
        {
            ...
            Database dbClient = new Database(true, false, 8);
        ...
        public static int lastItemID
        {
            get
            {
                Database dbClient = new Database(true, true, 9);
                return dbClient.getInteger("SELECT MAX(id) FROM furniture");
            }
        }
        public static string tradeItemList(int[] itemIDs)
        {
            ...
            for (int i = 0; i < itemIDs.Length; i++)
            {
                ...
                Database dbClient = new Database(true, true, 10);
                DataRow dRow = dbClient.getRow("SELECT tid, var FROM furniture WHERE id = '" + itemIDs[i] + "'");
